...
use crate::{binfmt::SectionMode, preprocessor::PreProcessor};
...
fn set_entrypoint(pp: &mut PreProcessor, input: Vec<&str>) -> Result<Vec<String>, String> {
    if input.len() != 1 {
...
pub fn section(pp: &mut PreProcessor, input: Vec<&str>) -> Result<Vec<String>, String> {
    if input.len() == 1 {
        ...
        let kind = SectionMode::from_str(input.get(2).unwrap()).map_err(|e| e.to_string())?;
        if kind == SectionMode::Heap {
            return Err("cannot create heaqp using this macro, use `.heap` instead".to_string());
        }
...
pub fn add_heap(pp: &mut PreProcessor, input: Vec<&str>) -> Result<Vec<String>, String> {
    if input.len() != 2 {
        return Err("format <offset> <value>".to_string());
    }
    let (offset_num, offset_base) = input.first().map(|s| get_base(s)).unwrap();
    let offset =
        u32::from_str_radix(offset_num, offset_base).map_err(|e| format!("parse offset: {}", e))?;
